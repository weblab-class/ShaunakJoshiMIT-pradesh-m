...
router.post("/appoint", async (req, res) => {
  ...
  try {
    ...
    if (user.nickname !== currentPresidentNickname) return res.status(403).json({ error: "Only the current president can appoint" });
    if (game.phase !== "APPOINT") return res.status(403).json({ error: "Cannot appoint while in this phase" });
    const appointeeUser = await User.findOne({ nickname: appointee });
    if (!appointeeUser) return res.status(404).json({ error: "Appointee user not found" });
    const isAppointeeInGame = game.user_ids.includes(appointeeUser.nickname);
    if (!isAppointeeInGame) return res.status(400).json({ error: "Appointee is not in the game" });
    ...
    return res.status(200).json({ message: "Hacker appointed successfully", appointee: appointeeUser.nickname });
  } catch (error) {
...
router.post("/vote", async (req, res) => {
  ...
  try {
    const game = await Game.findOne({ lobbyCode });
    if (!game) return res.status(404).json({ error: "Game not found" });
    if (!game.appointedHacker) return res.status(400).json({ error: "No hacker has been appointed" });
    if (!game.votes) game.votes = {};
    if (game.votes[user_id]) return res.status(400).json({ error: "You have already cast your vote" });
    const user = await User.findById(user_id);
    if (!user) return res.status(404).json({ error: "User not found" });
...
router.post("/move", async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(404).json({ error: "User not found" });
    const userNickname = user.nickname;
    if (game.phase !== "MOVE") return res.status(403).json({ error: "Cannot move while in this phase" });
    if (game.hacker !== userNickname) return res.status(403).json({ error: "Only the hacker can move" });
    function isAdjacentViaEdges(g, currentNode, tgtNode) {
      return g.edges.some((edge) => {
        const { source, target } = edge;
        return (source === currentNode && target === tgtNode) || (target === currentNode && source === tgtNode);
      });
    }
    const currentNode = game.location || "0-0";
    const validMove = isAdjacentViaEdges(game, currentNode, targetNode);
    if (!validMove) return res.status(403).json({ error: `Invalid move from ${currentNode} to ${targetNode}.` });
...
router.post("/answer", async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(user_id);
    if (!user) return res.status(404).json({ error: "User not found" });
    if (user.nickname !== game.hacker) return res.status(403).json({ error: "Only the hacker can answer" });
    if (game.phase !== "TRIVIA") return res.status(403).json({ error: "Cannot answer while in this phase" });
    const io = getIo();
    game.phase = "RESULT";
...
router.post("/result", async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(user_id);
    if (!game) return res.status(404).json({ error: "Lobby not found" });
    if (game.phase !== "RESULT") return res.status(403).json({ error: "Cannot process result while in this phase" });
    if (!user || user.nickname !== game.hacker) return res.status(403).json({ error: "Only the hacker can submit the result" });
    ...
    if (answerRight) {
      ...
      if (!nodeExists) return res.status(400).json({ error: `Node ${nodeIdToDelete} does not exist in the game.` });
      game.nodes = game.nodes.filter((node) => node.id !== nodeIdToDelete);
      game.edges = game.edges.filter((edge) => edge.source !== nodeIdToDelete && edge.target !== nodeIdToDelete);
      game.nextLocation = null;
      game.phase = "APPOINT";
    }
    game.currTurn = (game.currTurn + 1) % game.user_ids.length;
...
router.get("/role", async (req, res) => {
  ...
  try {
    const game = await Game.findOne({ lobbyCode });
    const user = await User.findById(user_id);
    if (!game || !user) return res.status(404).json({ error: "Lobby or user not found" });
    if (game.imposters.includes(user.nickname)) return res.status(200).json({ role: "FBI" });
    else return res.status(200).json({ role: "HACKER" });
  } catch (error) {
